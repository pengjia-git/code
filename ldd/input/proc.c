...
static int __init button_init(void)
{
	...
	printk("button_init jia\n");
	proc_bus_input_dir=proc_mkdir("jia",NULL);
	proc_mkdir("jia1",proc_bus_input_dir);
	if(proc_bus_input_dir){
		printk("create /proc/jia success\n");
	}
	else{
		printk("create /proc/jia fail\n");
	}
	return 0;
	//分配input设备
...
{
	remove_proc_entry("jia/jia1", NULL);
	remove_proc_entry("jia",NULL);
//	input_free_device(btn_dev);
